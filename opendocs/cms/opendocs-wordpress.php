...
class Wordpress_IDocs implements IDocs_CRUD {
	...
	*/
	public function insertItem($items, $itemIDs, $isCRON = false) {
			...
			foreach( $fieldMappings as $fieldMapping ) : 
				if( property_exists( $fieldMapping, 'type' ) ) : 
					if( $fieldMapping->type == 'repeater' ) : 
						$mappingArray[] = array( $fieldMapping->field_id, $fieldMapping->value, $fieldMapping->collectionID, 'sub_fields' =>$fieldMapping->sub_fields, 'field_type' => $fieldMapping->type, $fieldMapping->acf_name, $fieldMapping->sub_field_names );
					else : 
						$mappingArray[] = array( $fieldMapping->field_id, $fieldMapping->value, $fieldMapping->collectionID, 'field_type' => $fieldMapping->type );
					endif;
				else : 
					$mappingArray[] = array( $fieldMapping->field_id, $fieldMapping->value, $fieldMapping->collectionID, $fieldMapping->acf_name );
				endif;
			endforeach;
			...
			foreach( $mappingArray as $fieldMapping ) : 
		    	if( array_key_exists( 'field_type', $fieldMapping ) ) : 
					if( $fieldMapping['field_type'] == 'repeater' ) : 
						$toInsertMapping[] = array( 'field_id' => $fieldMapping[0], 'field_name' => $fieldMapping[1], 'collectionID' => $fieldMapping[2], 'sub_fields' => $fieldMapping['sub_fields'], 'field_type' => $fieldMapping['field_type'], 'acf_name' => $fieldMapping[3], 'sub_field_names' => $fieldMapping[4] );
					else : 
    					$toInsertMapping[] = array( 'field_id' => $fieldMapping[0], 'field_name' => $fieldMapping[1], 'collectionID' => $fieldMapping[2], 'field_type' => $fieldMapping['field_type'] );
					endif;
    			else : 
					$toInsertMapping[] = array( 'field_id' => $fieldMapping[0], 'field_name' => $fieldMapping[1], 'collectionID' => $fieldMapping[2], 'acf_name' => $fieldMapping[3] );
	    		endif;
		...
		$itemInfo = array( 'itemsList' => $itemList, 'itemHandles' => $itemHandleList, 'existingItems' => $existingItems );
	...
	*/
	public function insertCollectionInDB($postTypeInfo, $fieldMappings, $rejectedItems, $hasFileURL) {
		...
		$cronID = 0;
		$options = array( 'postType' => $postTypeInfo->postType, 'postTypeName' => $postTypeInfo->postTypeName, 'notifyEmail' => $notifyEmail, 'postStatus' => $postTypeInfo->postStatus, 'frequency' => $postTypeInfo->frequency, 'when' => $postTypeInfo->when, 'hasFileURL' => $hasFileURL );
		...
		$wpdb->insert( $tableName, 
			array( 	'collectionID' => $postTypeInfo->collectionID,
	...
	*/
	public function updateImportJob($items) {
		...
		foreach( $selectedPostTypes as $postType ) : 
			...
			$existingList = [];
			$postIDs = [];
			$options = array( 'postType' => $postType->postType, 'notifyEmail' => $notifyEmail, 'postStatus' => $postType->postStatus, 'frequency' => $postType->frequency, 'when' => $postType->when, 'hasFileURL' => $postType->hasFileURL );
	...
	public function getCRONSchedule() {
		...
		foreach( $importList as $import ) : 
			...
			if( $importOptions->frequency !== 'immediately' ) : 
				...
				$importSchedule[] = array( 'type' => $importOptions->frequency, 'ID' => $import->id, 'name' => $import->collectionName, 'postType' => $importOptions->postType, 'postStatus' => $importOptions->postStatus, 'email' => $importOptions->notifyEmail, 'collectionID' => $import->collectionID, 'mapping' => $fieldMappings, 'frequency' => $importOptions->frequency, 'when' => $importOptions->when[0], 'day' => $weekDays[$dayIndex] );
			endif;
		endforeach;
	...
	public function getItemTitle($itemID) {
    		...
    		$itemLoop = new WP_Query( array( 'post_type' => $postTypeList, 'meta_key' => 'odocs_item_id', 'meta_value' => $itemID ) );
	...
	private function getPostObjByItemID($itemID, $postType, $isGetID = false) {
		...
		while( $itemLoop->have_posts() ) : $itemLoop->the_post();
		if( ! $isGetID ) : 
			$postObj = array( 'id' => get_the_ID(), 'source' => get_post_meta( get_the_ID(), 'odocs_item_handle', true ), 'title' => get_the_title(), 'date' => get_the_date( 'Y-m-d H:i:s' ), 'edit' => get_edit_post_link( get_the_ID() ) );
		else : 
			$postObj = get_the_ID();
		endif;
		endwhile;
	...
	public function getPostIDsByItemIDs($items) {
		...
		foreach( $items as $item ) : 
			...
			if( ! empty( $notImportedItemIDs ) ) : 
				$importedItemLoop = new WP_Query( array( 'post_type' => $item->postType, 'posts_per_page' => -1, 'meta_query' => array( array( 'key' => 'odocs_item_id', 'value' => $notImportedItemIDs, 'compare' => 'IN', 'type' => 'NUMERIC' ) ) ) );
				while( $importedItemLoop->have_posts() ) : $importedItemLoop->the_post();
					$postObj[] = array( 'itemID' => (int)get_post_meta( get_the_ID(), 'odocs_item_id', true ), 'existing' => 0, 'insertedPost' => array( 'id' => get_the_ID(), 'source' => get_post_meta( get_the_ID(), 'odocs_item_handle', true ), 'title' => get_the_title(), 'date' => get_the_date( 'Y-m-d H:i:s' ), 'edit' => get_edit_post_link( get_the_ID() ) ) );
				endwhile;
				wp_reset_postdata();
				wp_reset_query();
			endif;
			$existingItemLoop = new WP_Query( array( 'post_type' => $postTypeList, 'posts_per_page' => -1, 'meta_query' => array( array( 'key' => 'odocs_item_id', 'value' => $existingItemIDs, 'compare' => 'IN', 'type' => 'NUMERIC' ) ) ) );
			while( $existingItemLoop->have_posts() ) : $existingItemLoop->the_post();
				$postObj[] = array( 'itemID' => (int)get_post_meta( get_the_ID(), 'odocs_item_id', true ), 'existing' => 1, 'existingPost' => array( 'id' => get_the_ID(), 'source' => get_post_meta( get_the_ID(), 'odocs_item_handle', true ), 'title' => get_the_title(), 'date' => get_the_date( 'Y-m-d H:i:s' ), 'edit' => get_edit_post_link( get_the_ID() ) ) );
			endwhile; 
			wp_reset_postdata();
			wp_reset_query();
		endforeach;
	...
	public function getFieldLabels($onlyIDs = false) {
		...
		foreach( $fieldLabelRows as $fieldLabelRow ) : 
			$fieldLabels[] = array( 'id' => $fieldLabelRow->id, 'fieldName' => $fieldLabelRow->fieldName, 'fieldLabel' => $fieldLabelRow->fieldLabel, 'fieldDesc' => $fieldLabelRow->fieldDesc );
		endforeach;
		return $fieldLabels;
	}
