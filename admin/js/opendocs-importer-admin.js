	...
	var cronID = -1;
	 ...
	 $(function() {
		...
		// Show selected page on page number click.
		 ...
		 $( "#opendocs_import" ).on( 'click', function(e) {
			 ...
			 selectedPostType.push({ 'collectionID' : $( '.field-mapping' ).attr('data-collectionid'), 'jobName' : jobName, 'collectionName' : $("#sel-coll-name").val(), 'notifyEmail' : notifyEmail, 'postStatus': postStatus, 'postType' : $(".field-mapping .post_types").val(), 'postTypeName' : $(".field-mapping .post_types option:selected").text(), 'frequency' : $(".field-mapping").find('.radio-when:checked').val(), 'when' : schedule_hour_day });
			 $(".mapping-table .table-row").each(function(index) {
				 ...
				 if( $(this).find(".table-left select").val() !== 'not-selected' || $(this).find(".odocs-metadata").val() !== 'not-selected' ) {
				 	if( dataType ) {
					 	switch (dataType) {
							case 'taxonomy': 
								postMapping.push({ 'field_id' : $(this).find(".table-left select").val(), 'collectionID' : $(this).parents('.field-mapping').attr('data-collectionid'), 'value' : $(this).find(".odocs-metadata").val(), 'type' : $(this).find(".table-left select option:selected").attr('data-field-type') });
								break;
							case 'repeater':
								postMapping.push({ 'field_id' : $(this).find(".table-left select").val(), 'collectionID' : $(this).parents('.field-mapping').attr('data-collectionid'), 'value' : $(this).find(".odocs-metadata").val(), 'sub_fields' : $(this).find(".table-left select option:selected").attr('data-sub-fields'), 'type' : $(this).find(".table-left select option:selected").attr('data-field-type'), 'acf_name' : $(this).find(".table-left select option:selected").attr('data-field-name'), 'sub_field_names' : $(this).find(".table-left select option:selected").attr('data-subfield-names') });
								break;
						}
				 	}
				 	else {
				 	 postMapping.push({ 'field_id' : $(this).find(".table-left select").val(), 'collectionID' : $(this).parents('.field-mapping').attr('data-collectionid'), 'value' : $(this).find(".odocs-metadata").val(), 'acf_name' : $(this).find(".table-left select option:selected").attr('data-field-name') });
				 	}
				 }
			 });
				 ...
				 if($(this).find(".item-select").eq(0).find("input:checkbox").is(':checked')) {
				 	if( ignoreIDs.indexOf($(this).find(".item-select").eq(0).find("input:checkbox").val()) == -1 ) {
						if( $(this).attr('existing') == "0" ) {
					 		toImportIDs.push({ 'id' : $(this).find(".item-select").eq(0).find("input:checkbox").val(), 'collectionID' : $(this).attr('data-collectionid') });
							toImportItemIDs.push( $( this ).find( ".item-select" ).eq( 0 ).find( "input:checkbox" ).val( ) );
							toImportCount++;
						}
						if( $(this).attr('existing') == "1" ) {
					 		existingItemIDs.push( { 'id' : $(this).find(".item-select").eq(0).find("input:checkbox").val(), 'collectionID' : $(this).attr('data-collectionid') } );
			 ...
			 toImportPostItemIDs.push({ 'postType' : $( ".field-mapping .post_types" ).val(), 'collectionID' : $('.field-mapping').attr('data-collectionid'), 'itemIDs': toImportItemIDs, 'existingItems': existingItemList });
			 ...
			 console.log("To Import Count: " + newItemCount + ", of: " + totalRecords + ", existing: " + existingItemCount);
				 ...
				 var dataToImport = { 'postType' : selectedPostType,
									 'postMapping' : postMapping,
									 'itemID' : toImportIDs,
									 'existingItemIDs' : existingItemList,
									 'allItems' : $("#allitemIDs").html()
									};
				 var datatoSend = { 'action' : 'insertItems',
			 	 ...
			 	 if( newItemCount > 0 ) { 
					$.ajax({url: ajaxurl,
						...
						success: function( response ) {
							callbackTimer = setTimeout(function() { checkForImportedPosts( toImportCount, newItemCount, existingItemCount ); }, 2000 );
							postOnlyImportTimer = setTimeout( function() { checkIfPostOnlyImportDone(); }, 1000 );
			 			}, 
					...
					$( ".form-wrap.existing-items-list" ).show();
				 	$( ".community-wrap .btn_wrap" ).hide();
				 	allItemsExported.push({ 'postType' : $( ".field-mapping .post_types" ).val(),
										   'itemIDs': allItemIDs,
			 ...
			 $.ajax({url: ajaxurl,
					...
					success: function( response ) {
						...
						if( response == newItemCount ) {
							...
							callbackTimer = setTimeout( function( ){ checkForImportedPosts( totalRecords, newItemCount, existingItemCount ); }, 5000 );
						}
					},
					error: function (jqXHR, exception) {
						callbackTimer = setTimeout( function( ){ checkForImportedPosts( totalRecords, newItemCount, existingItemCount ); }, 5000 );
			 ...
			 $.ajax({url: ajaxurl,
					...
					success: function(response) {
						if( response == 1 ) {
							...
							postOnlyImportTimer = setTimeout( function( ){ checkIfPostOnlyImportDone(); }, 5000 );
						}
					}
			});
		 }
		 ...
		 function checkforErrorImports() {
			 ...
			 $.ajax({url: ajaxurl,
					...
					timeout: 0,
					success: function(response) {
						errorImportTimer = setTimeout( function( ){ checkforErrorImports(); }, 10000 );
					}
			});
		 }
		 ...
		 function showImportedPosts( insertedPosts ) {
			...
			var newImportedItems = [];
		 	...
		 	schedules.push({ 'time': $( '.field-mapping' ).find( '.schedule-at #schedule-hour' ).val(), 'day': $('.field-mapping').find('.schedule-at #schedule-day').val(), 'frequency': $('.field-mapping').find('.radio-when:checked').val(), 'collName': $( ".job-title" ).val() });
		 ...
		 $(".abort-job a").on('click', function(e){
			 $('<div id="abortJob" title="Info"><p>Stop Import Job?</p></div>').dialog( {modal: true, buttons : [ { text: "Ok", click: function() { $(this).dialog("close");window.location.reload(); } }, { text: "Cancel", click: function() { $(this).dialog("close"); } } ] } );
			 e.preventDefault();
			 return false;
		 });
		 ...
		 $( ".btn_save .opendoc_btn" ).on( 'click', function(e) {
			 ...
			 $(".field-mapping .post_types").each(function(index) {
			 	...
			 	saveSelPostType.push({ 'collectionID' : $(this).parent().parent().parent().attr('data-collectionid'), 'collectionName' : collName, 'notifyEmail' : notifyEmail, 'postStatus': postStatus, 'postType' : $(this).val(), 'frequency' : $(this).parents(".field-mapping").find('.radio-when:checked').val(), 'when' : schedule_hour_day });
			 });
			 $(".mapping-table .table-row").each(function(index) {
				 var dataType = $(this).find(".table-left select option:selected").attr('data-field-type');
				 if( $(this).find(".table-left select").val() !== 'not-selected' || $(this).find(".odocs-metadata").val() !== 'not-selected' ) {
				 	if( dataType ) {
					 	switch (dataType) {
							case 'taxonomy': 
								savePostMapping.push({ 'field_id' : $(this).find(".table-left select").val(), 'collectionID' : $(this).parents('.field-mapping').attr('data-collectionid'), 'field_name' : $(this).find(".odocs-metadata").val(), 'field_type' : $(this).find(".table-left select option:selected").attr('data-field-type') });
								break;
							case 'repeater':
								savePostMapping.push({ 'field_id' : $(this).find(".table-left select").val(), 'collectionID' : $(this).parents('.field-mapping').attr('data-collectionid'), 'field_name' : $(this).find(".odocs-metadata").val(), 'sub_fields' : $(this).find(".table-left select option:selected").attr('data-sub-fields'), 'field_type' : $(this).find(".table-left select option:selected").attr('data-field-type'), 'acf_name' : $(this).find(".table-left select option:selected").attr('data-field-name'), 'sub_field_names' : $(this).find(".table-left select option:selected").attr('data-subfield-names') });
								break;
						}
				 	}
				 	else {
				 	 savePostMapping.push({ 'field_id' : $(this).find(".table-left select").val(), 'collectionID' : $(this).parents('.field-mapping').attr('data-collectionid'), 'field_name' : $(this).find(".odocs-metadata").val(), 'acf_name' : $(this).find(".table-left select option:selected").attr('data-field-name') });
				 	}
				 }
			 });
			 var dataToImport = { 'postType' : saveSelPostType, 'cronID' : cronID, 'jobName' : jobName, 'postMapping' : savePostMapping };
			 	 ...
			 	 $.ajax({url: ajaxurl,
						...
						success: function(response) {
							...
							if( response > 0 ) {
								...
								$('<div id="saveSuccess" title="Info"><p>Changes Saved</p></div>').dialog( {modal: true, buttons : [ { text: "Ok", click: function() { $(this).dialog("close"); } } ] } );
							}
	 ...
	 $( window ).load(function() {
	 	$(".opendocs-communities").on('click', 'a', function(e){
	 		...
	 		if( ! $(this).hasClass("loaded") ) {
	 			$.ajax({url: ajaxurl,
	 				...
	 				var data = { 'action': 'getCollections',
								'data': communityID,
							   };
  	 		 			$.ajax({url : ajaxurl, type : "POST", data : data, dataType: 'json', success : function(response1) {
		...
		$( ".btn_post_mapping button" ).on('click', function(){
	 	...
	 	function parentElems(elements, _array) {
	 		if(_array === undefined) { _array = []; }
    			else{ _array.push(elements.innerText); }
		...
		$(".community-wrap").on('change', '.field-mapping .post_types', function(e) {
			...
			if($(this).val() != 'sel') {
				...
				$.ajax( {
						...
						success: function(response1) {
							cfields = [];
							coreFields = {'dc.title' : 'Title', 'dc.date.accessioned' : 'Date', 'dc.description.abstract' : 'Content', 'dc.identifier.ag' : 'IDS Identifier'};
							coreFieldsLength = 0;
		...
		$(".community-wrap").on('click', '.del-mapping', function(e) {
			...
			$('<div id="saveSuccess" title="Info"><p>Delete Mapping?</p></div>').dialog( {modal: true, buttons : { "Ok": function() { $thisRow.remove();  $(this).dialog("close"); } , "Cancel": function() { $(this).dialog("close"); } } } );
			e.preventDefault();
			return false;
		});
